...
public static class GoalEndpoints
{
    public static void MapGoalEndpoints(this WebApplication app)
    {
            ...
            SyncRequest<GoalDto> req, ClaimsPrincipal user, AppDbContext db, JwtService jwt) =>
        {
            ...
            if (req.Records.Any(r => r.Category?.Length > 50))
            {
                logger.LogWarning("sync/goal account={Account} rejected: Category too long", accountGuid[..8]);
                return Results.Problem("Record Category must not exceed 50 characters.", statusCode: 422);
            }
